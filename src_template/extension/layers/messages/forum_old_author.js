...
(function () {
    ...
    OptionsFacade.prototype.createConnection = function () {
        var port = browser.runtime.connect({name: "background-options-forumold-author"});
        port.onMessage.addListener((message) => {
            if (message.command == "setoption") {
                this.optionsAuthor.unhideAuthorMessages = message.value;
            }
        });
    ...
    OptionsFacade.prototype.optionUnhideAuthorMessagesIsEnabled = function () {
        return this.optionsAuthor.unhideAuthorMessages;
    };
    ...
    PostFacade.prototype.unhideAuthorMessage = function () {
        if (this.node.isProcessed_unhideAuthorMessage) {
            return true;
        }
        else {
            this.node.isProcessed_unhideAuthorMessage = true;
        }
        ...
        if (spanNodes.length > 1
            ...
            .innerText.includes("скрыто автором темы")) {
            spanNodes[0].style.display = "block";
            spanNodes[1].style.display = "none";

            (function addAuthorMark(node) {
    ...
    function processPosts(options) {
        ...
        wrappedPosts.forEach((i) => {
            if (options.optionUnhideAuthorMessagesIsEnabled()) {
                i.unhideAuthorMessage();
            }
        });
    }
